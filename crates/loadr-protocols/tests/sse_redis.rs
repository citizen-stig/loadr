//! Integration tests for the SSE and Redis protocol handlers against the
//! in-process test servers.

use std::path::Path;
use std::sync::Arc;
use std::time::Duration;

use bytes::Bytes;
use loadr_config::HttpDefaults;
use loadr_core::data::DataFeeds;
use loadr_core::metrics::{MetricRegistry, MetricsBus, Tags};
use loadr_core::protocol::{PreparedRequest, ProtocolHandler, RequestOptions};
use loadr_core::vu::{RunContext, VuContext};
use loadr_protocols::{RedisHandler, SseHandler};
use loadr_testserver::{RedisTestServer, SseTestServer};

fn vu() -> VuContext {
    let data = DataFeeds::load(&Default::default(), Path::new(".")).expect("data feeds");
    let run = Arc::new(RunContext {
        variables: serde_json::Map::new(),
        secrets: Default::default(),
        env: Default::default(),
        data,
        registry: Arc::new(MetricRegistry::with_builtins()),
        base_dir: ".".into(),
        setup_data: parking_lot::RwLock::new(serde_json::Value::Null),
    });
    let (bus, _rx) = MetricsBus::new();
    VuContext::new(1, Arc::from("t"), Arc::new(Tags::new()), bus, run, true)
}

fn req(url: &str, protocol: &str) -> PreparedRequest {
    PreparedRequest {
        name: url.to_string(),
        protocol: protocol.to_string(),
        method: "GET".to_string(),
        url: url.to_string(),
        headers: Vec::new(),
        body: Bytes::new(),
        timeout: Duration::from_secs(10),
        follow_redirects: false,
        max_redirects: 0,
        options: RequestOptions::default(),
    }
}

// ---------------------------------------------------------------------------
// SSE
// ---------------------------------------------------------------------------

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn sse_streams_events_with_timings() {
    let server = SseTestServer::spawn().await.expect("sse server");
    let handler = SseHandler::new(&HttpDefaults::default(), Path::new(".")).expect("sse handler");
    let mut vu = vu();

    // Use the `sse://` alias to exercise scheme normalisation.
    let url = server.url().replacen("http://", "sse://", 1);
    let mut request = req(&url, "sse");
    request.options.plugin = Some(serde_json::json!({ "events": 5 }));

    let response = handler.execute(&mut vu, &request).await.expect("execute");

    assert!(
        response.error.is_none(),
        "unexpected sse error: {:?}",
        response.error
    );
    assert_eq!(response.status, 200);
    assert_eq!(response.protocol_version, "sse");

    let events_received = response.extras["events_received"].as_u64().unwrap();
    assert!(
        events_received >= 1,
        "expected events, got {events_received}"
    );
    assert_eq!(events_received, 5, "expected all 5 events");

    let last = &response.extras["last_event"];
    assert_eq!(last["type"], "tick");
    assert!(last["data"].as_str().unwrap().contains("\"n\":4"));

    assert!(response.bytes_received > 0);
    assert!(response.timings.duration_ms.is_finite());
    assert!(response.timings.duration_ms >= 0.0);
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn sse_until_substring_stops_early() {
    let server = SseTestServer::spawn().await.expect("sse server");
    let handler = SseHandler::new(&HttpDefaults::default(), Path::new(".")).expect("sse handler");
    let mut vu = vu();

    let mut request = req(&server.url(), "sse");
    request.options.plugin = Some(serde_json::json!({ "until": "\"n\":1" }));

    let response = handler.execute(&mut vu, &request).await.expect("execute");
    assert!(response.error.is_none());
    // Stops on the event whose data contains the substring (n=1 → 2 events).
    let events = response.extras["events_received"].as_u64().unwrap();
    assert_eq!(events, 2, "should stop at the matching event");
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn sse_connection_failure_is_reported() {
    let handler = SseHandler::new(&HttpDefaults::default(), Path::new(".")).expect("sse handler");
    let mut vu = vu();
    let request = req("sse://127.0.0.1:1/events", "sse");

    let response = handler.execute(&mut vu, &request).await.expect("execute");
    assert_eq!(response.status, 0);
    assert!(response.error.is_some());
    assert_eq!(response.protocol_version, "sse");
}

// ---------------------------------------------------------------------------
// Redis
// ---------------------------------------------------------------------------

fn redis_command(url: &str, args: &[&str]) -> PreparedRequest {
    let mut request = req(url, "redis");
    let command: Vec<serde_json::Value> = args.iter().map(|a| serde_json::json!(a)).collect();
    request.options.plugin = Some(serde_json::json!({ "command": command }));
    request
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn redis_set_get_reuses_connection() {
    let server = RedisTestServer::spawn().await.expect("redis server");
    let handler = RedisHandler::new();
    let mut vu = vu();
    let url = server.url();

    // SET establishes the pooled connection.
    let set = redis_command(&url, &["SET", "greeting", "hello"]);
    let set_resp = handler.execute(&mut vu, &set).await.expect("set");
    assert!(set_resp.error.is_none(), "set error: {:?}", set_resp.error);
    assert_eq!(set_resp.status, 0);
    assert_eq!(set_resp.body_text(), "OK");
    assert!(set_resp.timings.connect_ms > 0.0, "first call must connect");

    // GET reuses it — no fresh connect.
    let get = redis_command(&url, &["GET", "greeting"]);
    let get_resp = handler.execute(&mut vu, &get).await.expect("get");
    assert!(get_resp.error.is_none());
    assert_eq!(get_resp.status, 0);
    assert_eq!(get_resp.body_text(), "hello");
    assert_eq!(
        get_resp.timings.connect_ms, 0.0,
        "second call must reuse the pooled connection"
    );
    assert_eq!(get_resp.extras["reply_type"], "bulk");
    assert_eq!(get_resp.extras["value"], "hello");
    assert_eq!(get_resp.protocol_version, "redis");
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn redis_ping_and_unknown_command() {
    let server = RedisTestServer::spawn().await.expect("redis server");
    let handler = RedisHandler::new();
    let mut vu = vu();
    let url = server.url();

    let ping = redis_command(&url, &["PING"]);
    let ping_resp = handler.execute(&mut vu, &ping).await.expect("ping");
    assert!(ping_resp.error.is_none());
    assert_eq!(ping_resp.status, 0);
    assert_eq!(ping_resp.body_text(), "PONG");
    assert_eq!(ping_resp.extras["reply_type"], "string");

    let bogus = redis_command(&url, &["FROBNICATE", "x"]);
    let bogus_resp = handler.execute(&mut vu, &bogus).await.expect("bogus");
    assert_ne!(
        bogus_resp.status, 0,
        "error reply must have non-zero status"
    );
    assert!(bogus_resp.error.is_some());
    assert_eq!(bogus_resp.extras["reply_type"], "error");
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn redis_body_fallback_command() {
    let server = RedisTestServer::spawn().await.expect("redis server");
    let handler = RedisHandler::new();
    let mut vu = vu();

    // No plugin command: use the body as a space-separated command.
    let mut request = req(&server.url(), "redis");
    request.body = Bytes::from_static(b"PING");
    let resp = handler.execute(&mut vu, &request).await.expect("ping");
    assert!(resp.error.is_none());
    assert_eq!(resp.body_text(), "PONG");
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn redis_connection_failure_is_reported() {
    let handler = RedisHandler::new();
    let mut vu = vu();
    let request = redis_command("redis://127.0.0.1:1", &["PING"]);

    let response = handler.execute(&mut vu, &request).await.expect("execute");
    assert_eq!(response.status, 0);
    assert!(response.error.is_some());
    assert_eq!(response.protocol_version, "redis");
}
