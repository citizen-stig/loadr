...
run_one() {  # $1 = example file (in RUNDIR), $2.. = extra loadr args
  ...
  local reqs; reqs="$(echo "$out" | grep -oE '(http_reqs|plugin_reqs|grpc_reqs|postgres_reqs|mysql_reqs|mongo_reqs|kafka_reqs|ws_msgs_received)\.+: [0-9]+' | grep -oE '[0-9]+' | head -1)"
